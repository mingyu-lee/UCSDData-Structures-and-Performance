...
public abstract class Document_bak {
	 ...
	 */
	protected Document_bak(String text)
	{
		this.text = text;
	}
	...
	protected int countSyllables(String word)
	{
		...
		int cnt = 0;
		if(tmpWord[tmpWord.length-1] == 'e') {
			for(int wordIdx = 0; wordIdx < tmpWord.length-1; wordIdx++) {
				if(chkSyllables(tmpWord[wordIdx])) {
					int seqCnt = 0;
					if(tmpWord.length > 1 && wordIdx != tmpWord.length-2 && chkSyllables(tmpWord[wordIdx+1])) {
						seqCnt++;
						for(int chkSeqIdx = wordIdx+2; chkSeqIdx < tmpWord.length-2; chkSeqIdx++) {
							if(chkSyllables(tmpWord[chkSeqIdx])) {
								seqCnt++;
							}
						}
					}
			...
			for(int wordIdx = 0; wordIdx < tmpWord.length; wordIdx++) {
				if(chkSyllables(tmpWord[wordIdx])) {
					int seqCnt = 0;
					if(tmpWord.length > 1 && wordIdx != tmpWord.length-1 && chkSyllables(tmpWord[wordIdx+1])) {
						seqCnt++;
						for(int chkSeqIdx = wordIdx+2; chkSeqIdx < tmpWord.length-1; chkSeqIdx++) {
							if(chkSyllables(tmpWord[chkSeqIdx])) {
								seqCnt++;
							}
						}
	...
	private boolean chkSyllables(char character) {
		char[] syllables = { 'a', 'e', 'i', 'o', 'u', 'y'};
		for(int idx = 0; idx < syllables.length; idx++) {
			if(character == syllables[idx]) {
				return true;
			}
		}
		return false;
	 ...
	 */
	public static boolean testCase(Document_bak doc, int syllables, int words, int sentences)
	{
		...
		if (passed) {
			...
			System.out.println("fleshScore > " + fleshScore);
		}
		else {
